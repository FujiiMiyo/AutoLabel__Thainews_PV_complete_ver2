...
if __name__ == '__main__':
    ...
    ######### Note ##########
    #******---- Configuration ----******#
    target_word = 'วิศวกรรม' #<=============================== * Target word * #มัลแวร์
    target_doc = 'it_98' #<================================== * Target paragraph * #it_1500
    #******-----------------------******#
    #########################
    ...
    ####### Note ##########
    patterns = '(it_\w+)' #<====================================== Edit: economic, education, entertainment, foreign, it, sports 
    
    #-------- Feature vector [Deepcut] ----------#
    dsimdocs_id = [dtarget_doc_vector]
    for s, simdoc_id in enumerate (dsim_docs_id):
        z = re.match(patterns, simdoc_id)
        if z:
            #print("addsim_ddocs = ", z.groups())
            #add_sim = ' + ddoc_sim_doc_vector[' + str(s) + ']'
            add_sim = ddoc_sim_doc_vector[s]
            dsimdocs_id.append(add_sim)
        else:
            #print("subsim_ddocs = ", z.groups())
            #sub_sim = ' - ddoc_sim_doc_vector[' + str(s) + ']'
            sub_sim = -ddoc_sim_doc_vector[s]
            dsimdocs_id.append(sub_sim)
            
    dsimdocs_sum = sum(dsimdocs_id)
    #print("dsimdocs_sum = ", dsimdocs_sum)
    #print('dfeat = ', dsimdocs_id)        
    #print("ddd_sim = ", ddd_sim)
    #print("sub_sim = ", sub_sim)
    #print("d_sim_docs_id = ", simdoc_id)
    
    #-------- Feature vector [Attacut] ----------#
    asimdocs_id = [atarget_doc_vector]
    for s, simdoc_id in enumerate (asim_docs_id):
        z = re.match(patterns, simdoc_id)
        if z:
            #print("addsim_adocs = ", z.groups())
            #add_sim = ' + adoc_sim_doc_vector[' + str(s) + ']'
            add_sim = adoc_sim_doc_vector[s]
            asimdocs_id.append(add_sim)
        else:
            #print("subsim_adocs = ", z.groups())
            #sub_sim = ' - adoc_sim_doc_vector[' + str(s) + ']'
            sub_sim = -adoc_sim_doc_vector[s]
            asimdocs_id.append(sub_sim)
            
    asimdocs_sum = sum(asimdocs_id)
    #print("asimdocs_sum = ", asimdocs_sum)
    #print('afeat = ', asimdocs_id)        
    #print("add_sim = ", add_sim)
    #print("sub_sim = ", sub_sim)
    #print("a_sim_docs_id = ", simdoc_id)
    #######################
    ...
    #dfeat_vec1 = dtarget_doc_vector + ddoc_sim_doc_vector[0] + ddoc_sim_doc_vector[1] + ddoc_sim_doc_vector[2] + ddoc_sim_doc_vector[3] + ddoc_sim_doc_vector[4] + ddoc_sim_doc_vector[5] + ddoc_sim_doc_vector[6] + ddoc_sim_doc_vector[7] + ddoc_sim_doc_vector[8] + ddoc_sim_doc_vector[9]
    dfeat_vec = dsimdocs_sum
    ...
    #afeat_vec1 = atarget_doc_vector + adoc_sim_doc_vector[0] + adoc_sim_doc_vector[1] + adoc_sim_doc_vector[2] + adoc_sim_doc_vector[3] + adoc_sim_doc_vector[4] + adoc_sim_doc_vector[5] + adoc_sim_doc_vector[6] + adoc_sim_doc_vector[7] + adoc_sim_doc_vector[8] + adoc_sim_doc_vector[9]
    afeat_vec = asimdocs_sum
